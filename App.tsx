...
const App: React.FC = () => {
  ...
  const startApp = async () => {
    ...
    try {
      ...
      // 2. URL Cleanup: Redirect stray paths to root if not using token or api
      // This fixes "typing anything after domain opens app" confusion by snapping to root
      if (window.location.pathname !== '/' && !window.location.pathname.startsWith('/api') && !token) {

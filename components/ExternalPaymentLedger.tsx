...
export const ExternalPaymentLedger: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-20 animate-fadeIn">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-3 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 relative animate-scaleUp max-h-[90vh] max-h-[90dvh] flex flex-col overflow-hidden">
            {/* Minimal Header */}
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-900 tracking-tight">New Payment Request</h2>
              </div>
              <button
            ...
            {/* Form Body */}
            <form id="create-payment-form" onSubmit={handleCreateRequest} className="flex-1 overflow-y-auto p-4 space-y-2.5">
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:outline-none focus:ring-1 focus:ring-amber-500 focus:bg-white"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-900 focus:outline-none focus:ring-1 focus:ring-amber-500 focus:bg-white"
                  />
                </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Counter sale / Repair charges"
              ...
              {/* Minimal Streamlined Ref ID Bar */}
              <div className="flex items-center justify-between px-3 py-2 bg-amber-50/80 border border-amber-200/80 rounded-lg">
                <div className="flex items-center gap-1.5 min-w-0">
                  <Lock size={12} className="text-amber-600 shrink-0" />
                  <span className="text-[10px] font-bold uppercase tracking-wider text-amber-800 shrink-0">Ref ID:</span>
                  <span className="font-mono font-bold text-amber-950 text-xs truncate">{systemGeneratedRefId}</span>
                </div>
                <button
                  type="button"
                  onClick={() => copyToClipboard(systemGeneratedRefId, 'System Reference ID', 'new-ref-id')}
                  className="text-amber-800 hover:text-amber-950 text-[10px] font-bold flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-amber-100 transition-colors shrink-0 cursor-pointer"
                >
                  {copiedId === 'new-ref-id' ? <Check size={11} className="text-emerald-600" /> : <Copy size={11} />}
              ...
              <div className="pt-0.5">
                <label className="flex items-center gap-2 text-xs font-medium text-slate-700 cursor-pointer select-none">
                  <input
                    ...
                    className="w-3.5 h-3.5 rounded text-amber-600 focus:ring-amber-500 shrink-0"
                  />
                  <span>Dispatch WhatsApp link immediately</span>
                </label>
              </div>
            </form>

            {/* Minimal Footer */}
            <div className="px-4 py-2.5 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-2 shrink-0">
              <button
                type="button"
                onClick={() => setShowCreateModal(false)}
                className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 font-semibold text-xs rounded-lg border border-slate-200 transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-1.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs rounded-lg shadow-sm transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
                {isGeneratingSetu ? (
                  <>

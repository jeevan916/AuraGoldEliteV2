          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 relative text-center animate-scaleUp">
            ...
            <div className="space-y-2 text-left bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs my-4">
              ...
              <div className="flex justify-between text-slate-600">
                <span>Pay Link:</span>
                <a
                  href={showQrModal.shortLink || (showQrModal.platformBillID ? `https://setu.co/upi/s/${showQrModal.platformBillID}` : `${window.location.origin}/?token=${showQrModal.shareToken}`)}
                  target="_blank"
                  rel="noreferrer"
                  className="font-mono text-amber-600 font-bold truncate max-w-[180px] hover:underline"
                >
                  {showQrModal.shortLink || (showQrModal.platformBillID ? `https://setu.co/upi/s/${showQrModal.platformBillID}` : `${window.location.origin}/?token=${showQrModal.shareToken}`)}
                </a>
              </div>
            </div>
